...
export default class DresserMale extends React.Component {
    ...
    captureAndSend = async () => {
        ...
        if (this.camera) {
          ...
          //   if (Platform.OS == 'ios')
            var  cropHeight = 65/100 *screenHeight
             ...
             var   photoSelf = await ImageManipulator.manipulateAsync(photo.uri, [
...

                {
                    crop: {
                        ...
                        originY:0,
                        width: photo.width,
                        height:parseInt(75/100 *photo.height)
                    }
        ...
        function onReceive(event) {
            ...
            else if(isJson(event.data))
            {
               // alert(event.data);
                ...
                var isTransExist = false;
                document.canvasB = document.createElement('canvas');  
                          
                if(messgeObject.original)
                {
                   // alert(messgeObject.original);  
                    
                   var imageData = canvas.getContext("2d").getImageData(0, 0, canvas.width, canvas.height); //take away the .data
                   var r, g, b, a;
...
for(var i = 0; i+3 < imageData.data.length; i+=4) {
                 ...
                 if(!isTransExist &&  document.canvasB.toDataURL() != canvas.toDataURL() )
                 {
                     
                    
                    window.ReactNativeWebView.postMessage('{"title":"Aww.. no place to trial/capture","message":"Please select any cloth parts or draw"}');
                    ...
                    if(!imageBackup)
                {
                   // alert('line 997 and drawing initial image '+messgeObject.original);
                    image.src='data:image/jpg;base64,'+messgeObject.original;
                    imageBackup='data:image/jpg;base64,'+messgeObject.original;
                   
                    imageForBody = 'data:image/jpg;base64,'+messgeObject.compressed;
                    imageHistory.push(imageBackup);
                ...
                else
                {
                    ...
                    refreshCanvas('data:image/jpg;base64,'+messgeObject.original);
                    imageHistory.push('data:image/jpg;base64,'+messgeObject.original);
                    overrideLastImage = true;
